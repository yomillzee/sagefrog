...
def fetch_paid_microsoft_key_event_daily(
    property_id: str, start: str, end: str, access_token: str,
    *, client_key: str = "",
) -> list[dict[str, Any]]:
    """Microsoft/Bing key events split by event name, per campaign name.

    Microsoft Ads has no native GA4 link (that only exists for Google Ads via
    gclid auto-tagging), so matching is name-based at the campaign level —
    same convention as LinkedIn. Scoped to Bing sessions by source; keeps
    only key_events > 0. Joins to explorer_microsoft_ads_daily by normalized
    campaign_name in the reader.
    """
    try:
        rows = _run_report(
            property_id,
        ...
        _log.warning(
            "GA4 microsoft_key_event_daily skipped — API rejected dimension/metric combination "
            "or returned error. Table will be empty for this sync. Error: %s", exc,
        )
        return []
    synced_at = _now()
    ...
    for r in rows:
        ...
        campaign = r.get("sessionCampaignName") or ""
        key_events = int(float(r.get("keyEvents") or 0))
        if "bing" not in source and "microsoft" not in source:
            continue
        if campaign in ("", "(not set)") or key_events <= 0:
            continue
        out.append({
...
GA4_REPORTS = [
    ...
    {
        "report_key": "paid_microsoft_key_event",
        "table_name": "ga4_microsoft_key_event_daily",
        "dimensions": ["date", "sessionSource", "sessionCampaignName", "eventName"],
        "metrics": ["keyEvents"],
        "grain": ["date", "campaign_name", "event_name"],
        "required": False,
        "notes": "Microsoft/Bing key events split by event name, per campaign name. Name-matched to explorer_microsoft_ads_daily — Microsoft has no native GA4 link like Google Ads. Only key_events > 0 kept.",
    },
    {
