...
def load_harvest_env() -> HarvestEnv:
    return HarvestEnv(
...
def _resolve_refresh_token() -> str:
    try:
        ...
        db_token = oauth_store.get_refresh_token("harvest")
        if db_token:
            return db_token
    except Exception:
        pass
    raise RuntimeError(
        "Missing Harvest refresh token. Connect Harvest in Admin → Ad platform connections."
    )
...
def _has_refresh_token() -> bool:
    try:
        ...
        return bool(oauth_store.get_refresh_token("harvest"))
    except Exception:
        return False
