...
def email_performance_page_url(
    *,
    client_slug: str = "penn",
    access_key: str | None,
    use_session: bool,
) -> str | None:
    base = f"/dashboard/{client_slug}/email-performance"
    if use_session:
        return base
    if access_key:
        return f"{base}?key={quote(access_key, safe='')}"
...
def linkedin_organic_page_url(
    *,
    client_slug: str = "penn",
    access_key: str | None,
    use_session: bool,
) -> str | None:
    base = f"/dashboard/{client_slug}/linkedin-organic"
    if use_session:
        return base
    if access_key:
        return f"{base}?key={quote(access_key, safe='')}"
...
def accessibility_page_url(
    *,
    client_slug: str = "penn",
    access_key: str | None,
    use_session: bool,
) -> str | None:
    base = f"/dashboard/{client_slug}/accessibility"
    if use_session:
        return base
    if access_key:
        return f"{base}?key={quote(access_key, safe='')}"
