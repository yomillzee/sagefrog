...
Rendered inside the shared admin shell (navy sidebar + client switcher) so Budget
HQ sits alongside the other admin sections rather than as a standalone page. The
page shell paints instantly and fetches /admin/hq/data for the numbers, so a cold
BigQuery read per client never blocks first paint.
"""
...
def render_hq_budget_page(*, user_email: str) -> str:
    ...
    return render_admin_shell_page(
        active_nav="hq",
        page_title="Budget HQ",
        content_html=_HQ_CONTENT,
