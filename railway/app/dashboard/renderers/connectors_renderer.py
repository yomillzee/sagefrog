...
_PLATFORM_ICONS: dict[str, str] = {
    # LinkedIn — brand-blue tile with the white "in" glyph.
    "linkedin_ads": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<rect width="24" height="24" rx="4" fill="#0A66C2"/>'
        '<path fill="#fff" d="M8.34 18.34V9.9H5.55v8.44h2.79zM6.94 8.74a1.62 1.62 0 1 0 '
        "0-3.24 1.62 1.62 0 0 0 0 3.24zM18.45 18.34v-4.63c0-2.47-1.32-3.62-3.09-3.62-1.42 "
        "0-2.05.78-2.41 1.33V9.9h-2.79c.04.79 0 8.44 0 8.44h2.79v-4.71c0-.25.02-.5.09-.68.2-.5.66-1.02 "
        '1.42-1.02 1 0 1.4.76 1.4 1.88v4.53h2.79z"/></svg>'
    ),
    # Meta — the infinity mark in Meta blue.
    "meta_ads": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<path fill="#0866FF" d="M6.915 4.03c-1.968 0-3.683 1.28-4.871 3.113C.704 9.208 0 '
        "11.883 0 14.449c0 .706.07 1.369.21 1.973a6.624 6.624 0 0 0 .265.86 5.297 5.297 0 "
        "0 0 .371.761c.696 1.159 1.818 1.927 3.593 1.927 1.497 0 2.633-.671 3.965-2.444.76-1.012 "
        "1.144-1.626 2.663-4.32l.756-1.339.186-.325c.061.1.121.196.183.3l2.152 3.595c.724 1.21 "
        "1.665 2.556 2.47 3.314 1.046.987 1.992 1.22 3.06 1.22 1.075 0 1.876-.355 2.455-.843a3.743 "
        "3.743 0 0 0 .81-.973c.542-.939.861-2.127.861-3.745 0-2.72-.681-5.357-2.084-7.45-1.282-1.912-2.957-2.93-4.716-2.93-1.047 "
        "0-2.088.467-3.053 1.308-.652.57-1.257 1.29-1.82 2.05-.69-.875-1.335-1.547-1.958-2.056-1.182-.966-2.315-1.303-3.454-1.303zm10.16 "
        "2.053c1.147 0 2.188.758 2.992 1.999 1.132 1.748 1.647 4.195 1.647 6.4 0 1.548-.368 "
        "2.9-1.839 2.9-.58 0-1.027-.23-1.664-1.004-.496-.601-1.343-1.878-2.832-4.358l-.617-1.028a44.908 "
        "44.908 0 0 0-1.255-1.98c.07-.109.141-.224.211-.327 1.12-1.667 2.118-2.602 3.358-2.602zm-10.201.553c1.265 "
        "0 2.058.791 2.675 1.446.307.327.737.871 1.234 1.579l-1.02 1.566c-.757 1.163-1.882 "
        "3.017-2.837 4.338-1.191 1.649-1.81 1.817-2.486 1.817-.524 0-1.038-.237-1.383-.794-.263-.426-.464-1.13-.464-2.046 "
        "0-2.221.63-4.535 1.66-6.088.454-.687.964-1.226 1.533-1.533a2.264 2.264 0 0 1 "
        '1.087-.291z"/></svg>'
    ),
    # Google Ads — the overlapping yellow/blue bars with the blue pivot dot.
    "google_ads": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<rect x="6.5" y="1.8" width="6" height="17" rx="3" transform="rotate(-30 9.5 10.3)" fill="#FBBC04"/>'
        '<rect x="11.5" y="5.2" width="6" height="17" rx="3" transform="rotate(-30 14.5 13.7)" fill="#4285F4"/>'
        '<circle cx="6.4" cy="18.1" r="3" fill="#4285F4"/>'
        '</svg>'
    ),
    # Google Analytics 4 — the amber bar with the orange "9" mark.
    "ga4": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<rect x="14" y="2.5" width="6.2" height="19" rx="3.1" fill="#F9AB00"/>'
        '<rect x="3.5" y="10" width="6.2" height="11.5" rx="3.1" fill="#E8710A"/>'
        '<circle cx="6.6" cy="17.9" r="3.4" fill="#E8710A"/>'
        '</svg>'
    ),
    # Search Console — clean Google-blue magnifier.
    "gsc": (
        '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true">'
        '<circle cx="10.5" cy="10.5" r="7.4" stroke="#4285F4" stroke-width="2.6"/>'
        '<path d="M20.5 20.5l-5-5" stroke="#4285F4" stroke-width="2.8" stroke-linecap="round"/>'
        '</svg>'
    ),
    # Google Tag Manager — the blue tangram/tag diamond.
    "gtm": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<path fill="#8AB4F8" d="M12 1.6 1.6 12l5.2 5.2L17.2 6.8z"/>'
        '<path fill="#4285F4" d="M12.02 22.38 22.4 12l-5.2-5.2L6.82 17.18z"/>'
        '<circle cx="12" cy="12" r="2.35" fill="#246FDB"/>'
        '</svg>'
    ),
    # HubSpot — the sprocket in HubSpot orange.
    "hubspot": (
    ...
    # SEMrush — the orange app tile with a white performance gauge.
    "semrush": (
        '<svg viewBox="0 0 24 24" aria-hidden="true">'
        '<rect width="24" height="24" rx="5" fill="#FF642D"/>'
        '<path d="M6 15a6 6 0 0 1 12 0" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round"/>'
        '<path d="M12 15l3.4-2.4" stroke="#fff" stroke-width="2" stroke-linecap="round"/>'
        '<circle cx="12" cy="15" r="1.5" fill="#fff"/>'
        '</svg>'
    ),
    # PageSpeed Insights — the speed gauge in Google colors.
    "pagespeed": (
        '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true">'
        '<path d="M3.5 18a9 9 0 1 1 17 0" stroke="#5F6368" stroke-width="2.1" stroke-linecap="round"/>'
        '<path d="M12 12l4.2-3.4" stroke="#34A853" stroke-width="2.2" stroke-linecap="round"/>'
        '<circle cx="12" cy="12" r="2.1" fill="#EA4335"/>'
        '<path d="M4.6 12.2l1.7.4M19.4 12.2l-1.7.4M12 4.4v1.8" stroke="#FBBC04" stroke-width="1.8" stroke-linecap="round"/>'
        '</svg>'
    ),
}
