...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    /* The shared chrome (linked below) covers .dash-sidebar* only — the
       flex-container rules that lay the sidebar and content side by side
       normally live in render_client_shell_page's own style block, which this
       standalone page doesn't use, so they're declared explicitly here. */
    .app-shell {{ display: flex; flex-direction: row; min-height: 100vh; }}
    .dash-main {{ flex: 1 1 auto; min-width: 0; }}
  </style>
  <!-- Same cached chrome every other page links; see dashboard/assets.py. -->
