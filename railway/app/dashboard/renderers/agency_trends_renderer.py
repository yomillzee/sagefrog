...
Built the same way as the HQ budget page — a self-contained shell that mirrors
the admin chrome and fetches /admin/agency-trends/data for the numbers, so the
DuckDB rollup never blocks first paint. The page reproduces exactly what HQ
shows (every client's spend vs budget and its trailing-30d web-traffic
sparkline) and adds week-over-week paid-media momentum and channel mix on top,
all computed from one concurrent set of BigQuery reads by
dashboard.services.agency_trends_service (build_agency_overview).
"""
