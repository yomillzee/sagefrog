...
**Every client's dashboard renders through one master template** —
`bigquery_dashboard_renderer.py` (`render_bigquery_dashboard_page`). There are no
per-client dashboard renderers; an edit there applies to every dashboard. Per-client
differences (segment filters, mart destination, theme, feature flags) come from
configuration (`client_dashboard_config`), never from branching on a client's name.
See `railway/app/dashboard/ARCHITECTURE.md` for the full map.

| Renderer | Responsibility |
|---|---|
| `bigquery_dashboard_renderer.py` | **The master dashboard template** — every client's dashboard (Overview / Campaign Explorer / Website Analytics) |
| `base_layout.py` | Topbar, account chip, refresh toolbar, page shell, canonical sidebar nav |
...
on/off-track pace. A client that runs two distinct lines of work — a main retainer plus
a separate HR retainer, say — shouldn't be paced as one merged total: tick **Own card**
next to a project in a card's details (⋯) popout and it gets its own burn-up, titled by
the project and carrying its own goal, while the client's card keeps the rest of the
work. Untick it to fold the project (and its goal) back in. The page's **Share** button mints read-only, no-login links
